...
locals {
  gitlab_project_count = var.project_id == null || var.project_id == "" ? 0 : 1
  gitlab_group_count   = var.group_id == null || var.group_id == "" ? 0 : 1
}
...
locals {
  ...
  gitlab_project_path       = replace(coalesce(concat(data.gitlab_project.project.*.web_url, data.gitlab_group.group.*.web_url)...), "https://gitlab.skypicker.com/", "")
